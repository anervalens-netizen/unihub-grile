...
| AC-10 | Overview, filtrele, Program, Magazin, Agent și Excepții sunt coerente și utilizabile la 75+ magazine | fără Google I/O în web reads | component/e2e + query-count/perf; no N+1 | browser desktop/tablet/mobile | S4 builder packet landed at `82d481e` (UI surfaces, virtualized matrix, perf probe, worker job kinds); user review on autonomous run flagged incomplete Program cell edit/save handler, missing Close flow actions, Magazin/Agent missing contract fields, and Overview leaking tenant-wide info. AC-10 remains UNVERIFIED pending the S4 repair packet plus a real browser probe (Playwright not present in this sandbox). | UNVERIFIED |
