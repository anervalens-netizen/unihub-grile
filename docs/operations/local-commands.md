...
Canonical plan/status: issues #3 and #4.
...
Known hardening work for the program remains tracked under `JOB-*` in issue #4.
Do not interpret successful startup as proof that retry/recovery semantics are
already server-ready.
...
The current development identity mechanism is temporary. It may use
`X-Ugrile-Identity` / `X-Ugrile-Tenant` in local flows until `SEC-*` tasks replace
implicit fallbacks with an explicit environment-gated principal provider.
...
The exact semantics will be hardened under `OPS-002`; current successful probes
show current local runtime state only.
...
The endpoint enqueues work; run the worker to settle it.

`SEC-009` requires proof that this development route cannot be exposed
accidentally in production mode before server-test readiness.
...
- calendar UI → component + available browser proof;
