...
import { queueStateLabel, queueStateTone, type QueueState } from "../operationalStatus";
  ...
  return (
    <div className="command-page">
      <section className="command-hero">
        <div>
          <span className="eyebrow">OPERAȚIUNI / FUNDAL</span>
          <h2>Joburi și sincronizări</h2>
          <p>Coada activă, reîncercările și ultimele rezultate ale operațiunilor asincrone.</p>
        </div>
      ...
      {diagnostics && (
        <>
          <section className="kpi-strip" aria-label="Stare joburi">
            ...
            <Metric label="Reîncercare" value={diagnostics.counts.retrying} detail="așteaptă următoarea încercare" tone={diagnostics.counts.retrying > 0 ? "warn" : "ok"} />
            <Metric label="În execuție" value={diagnostics.counts.running} detail="execuție activă" tone={diagnostics.counts.running > 0 ? "warn" : "ok"} />
...
function scopeLabel(job: JobDiagnostic): string {
  if (job.store_ids.length === 0) return job.month_id ? "lună" : "organizație";
